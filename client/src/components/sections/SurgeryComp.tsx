...
const SurgeryComp: React.FC<SurgCompProps> = ({ id }) => {
    ...
    const [type, setType] = useState<string>('Eye');
    ...
    useEffect(() => {
        ...
        axios.get(`http://localhost:3000/api/Surgeries?patient_id=${id}`)
            .then(response => {
    ...
    const addSurg = () => {
        ...
        axios.post('http://localhost:3000/api/Surgery', newSurg)
            .then(response => {
    ...
    const editSurg = () => {
        ...
        axios.put(`http://localhost:3000/api/Surgery/${selectedSurg.id}`, updatedSurg)
            .then(response => {
    ...
    const deleteSurg = (id: number) => {
        axios.delete(`http://localhost:3000/api/Surgery/${id}`)
            .then(() => {
    ...
    const handleEdit = (surg: Surg) => {
        ...
        const surgeryDate = new Date(surg.surg_date);
        if (isValid(surgeryDate)) {
            setDate(surgeryDate);
            setTime(format(surgeryDate, 'HH:mm'));
            setNewStatus(surgeryDate.getHours() < 12 ? 'AM' : 'PM');
        } else {
            console.error('Invalid surgery date:', surg.surg_date);
        }
    };
                            ...
                            {data.map(surg => {
                                ...
                                return (
                                    <TableRow key={surg.id} className="border-t">
                                        ...
                                        <TableCell className="py-2">{isValid(surgeryDate) ? format(surgeryDate, 'PPP') : 'Invalid Date'}</TableCell>
                                        <TableCell className="py-2">{isValid(surgeryDate) ? format(surgeryDate, 'hh:mm a') : 'Invalid Time'}</TableCell>
